...
const FormUpdateNote = ({ noteFields }) => {
    ...
    const uploadNoteToFirebase = async (titleNote, descriptionNote) => {
        ...
        const matchingDoc = querySnapshot.docs.find((doc) => doc.data().titleNote === noteFields.titleNote)
        ...
        if (matchingDoc) {
            await updateDoc(matchingDoc.ref, dataNote);
            setTimeout(() => {
        ...
        // const addNote = updateDoc(notesCollectionRef, dataNote);
    };


    const [selectedPriorityOption, setSelectedPriorityOption] = useState(noteFields.priority);
    ...
    return (
        <Formik
        initialValues={{titleNote: noteFields.titleNote, descriptionNote: noteFields.descriptionNote,}}
        onSubmit={(values) => {
            animationsPopUp();
            uploadNoteToFirebase(values.titleNote, values.descriptionNote)
            // console.log(values);
            console.log('Tu nota se a actualizado con exito 🎉');
        }}
                    ...
                    <Animated.View style={[{position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, justifyContent: 'center', alignItems: 'center', zIndex: zIndex, }, colorBgPopUpStyle]}>
                        <Animated.View style={[{ position: 'absolute', top: 0, bottom: 0, left: 0, right: 0, justifyContent: 'center', alignItems: 'center', zIndex: zIndex, }, scalePopUpStyle]}>
                            <View style={{backgroundColor: selectedTheme.darkAccent, width: dimensions.width / 1.2, paddingVertical: 15, paddingHorizontal: 20, borderRadius: 20, alignItems: 'center',}}>
								<Text style={{fontFamily: 'Gilroy-SemiBold', fontSize: 17, color: selectedTheme.whiteColor}}>Tu nota se a actualizado con exito 🎉🥳</Text>
                            </View>
                        </Animated.View>
                    </Animated.View>
                </>
